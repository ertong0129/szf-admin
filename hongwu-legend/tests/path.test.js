...
assert.deepStrictEqual(D.NPC_TILES.jingche, [124, 67]);
assert.ok(D.MAP_SIZE.capital.w >= 140 && D.MAP_SIZE.capital.h >= 130);
assert.ok(D.PORTALS.capital.some(function (p) { return p.to === 'wild'; }), '京城应有往横涧山出口');
assert.ok(D.NPCS.jingtie && D.NPCS.jingtie.map === 'capital');
assert.ok(D.NPCS.jingzhishi && D.NPCS.jinwei);
