...
assert.strictEqual(tileMan.maps.jing_cheng.isoTile, 44);
assert.strictEqual(tileMan.maps.jing_cheng.offsetX, 3520);
assert.strictEqual(tileMan.maps.jing_cheng.offsetY, -1232);
...
assert.ok(ui.indexOf('radarToWalk') >= 0, '当前地图点击应按等距反算格子');
...
assert.ok(fs.readFileSync(path.join(root, 'assets/ingame/maptiles/manifest.json'), 'utf8').indexOf('"offsetX": 3520') >= 0, '京城 MCM offsetX 应为 3520');
