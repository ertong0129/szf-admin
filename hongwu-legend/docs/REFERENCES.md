...
场景大地图不是一张整图，而是公开 CDN 上的 300×300 JPEG 切片，规则为 `com/maps/{folder}/{row}_{col}.jpg`（例如京城 `jing_cheng/12_15.jpg` 是第 12 行、第 15 列）。按像素 `(col*300, row*300)` 轴对齐即可拼回。公开 `com/maps/mcm/13.mcms` 里 `13100.mcm` 给出京城行走网格 **175×172**、X 向原点偏移 **51** 格；拼图像素按这套等距公式铺，才能让 `pos.txt` 的史可法 `[115,36]` 落在游览 HAR 起点切片 `7_22`。HAR 一次镜头大约预加载 6×5 块（含缓冲），对应原作 1000 宽舞台对 300px 切片 **1:1**，镜头居中。顶栏活动钮用公开 `resources/smallIcon/` 里自带四字标题的 58×58 PNG。原 300×300 块只作本地缓存，不入库；拼好后缩到长边 2048，放在 `assets/ingame/maptiles/`。小地图仍用 `com/maps/smallMap` 俯视图。京城 44 个 type=4 NPC 对照公开 `pos.txt` 的 `11100/13100` 格子。`.mcm` 切片表多数 403，网格范围用 HEAD 探测；京城为 26×16 块、原生 7800×4800。
